<?php
// Alat: Topologi & Subnet (TKJ)
\App\Domain\Tkj\Topo::ensureTables($conn);
$tp_cidr = trim($_GET['cidr'] ?? ($_POST['cidr'] ?? '192.168.1.0/24'));
$tp_calc = $tp_cidr !== '' ? \App\Domain\Tkj\Topo::parse($tp_cidr) : null;
$tp_saved = [];
try {
    $q = $conn->prepare("SELECT id, name, created_at FROM topo_saves WHERE user_id = ? ORDER BY id DESC LIMIT 10");
    if ($q) { $q->bind_param("i", $uid); $q->execute(); $tp_saved = $q->get_result()->fetch_all(MYSQLI_ASSOC); $q->close(); }
} catch (Throwable $e) {}
?>
<div class="row g-4 align-items-start">
    <div class="col-lg-3">
        <section class="card p-3 mb-3 border-0 shadow-sm">
            ...
            <div class="mt-3 pt-3 border-top small">
                <div class="mb-1"><span class="text-muted d-block" style="font-size:0.7rem">Network</span><code class="text-info"><?= htmlspecialchars($tp_calc['network']) ?></code></div>
                <div class="mb-1"><span class="text-muted d-block" style="font-size:0.7rem">Broadcast</span><code class="text-info"><?= htmlspecialchars($tp_calc['broadcast']) ?></code></div>
                <div class="mb-1"><span class="text-muted d-block" style="font-size:0.7rem">Subnet Mask</span><code><?= htmlspecialchars($tp_calc['mask']) ?></code></div>
                <div class="mb-1"><span class="text-muted d-block" style="font-size:0.7rem">Total Host (Usable)</span><strong><?= (int)$tp_calc['hosts'] ?></strong></div>
                <div class="mt-2 text-muted" style="font-size:0.75rem">Kelas <?= htmlspecialchars($tp_calc['class']) ?> · <?= !empty($tp_calc['private']) ? 'Privat' : 'Publik' ?></div>
            </div>
            <?php elseif ($tp_calc): ?>
            <p class="small text-danger mt-2 mb-0"><?= htmlspecialchars($tp_calc['msg']) ?></p>
            <?php endif; ?>
        </section>
        <?php if ($tp_saved): ?>
        <section class="card p-3 border-0 shadow-sm">
            <h2 class="h6 fw-bold mb-2"><i class="fas fa-folder-open me-2"></i>Tersimpan (<?= count($tp_saved) ?>)</h2>
            <div class="d-flex flex-column gap-1">
                <?php foreach ($tp_saved as $s): ?>
    ...
    <div class="col-lg-9">
        <details class="card border-0 shadow-sm overflow-hidden p-3" open>
            <summary class="h6 fw-bold mb-0" style="cursor:pointer"><i class="fas fa-network-wired me-2"></i>Kanvas topologi — klik untuk buka/tutup</summary>
            <div class="mt-3">
        <section class="card border shadow-sm overflow-hidden p-0">
                ...
                <svg width="100%" height="100%" xmlns="http://www.w3.org/2000/svg" style="position: absolute; top:0; left:0; pointer-events: none;">
                    <defs>
                        <pattern id="grid" width="40" height="40" patternUnits="userSpaceOnUse">
                            <path d="M 40 0 L 0 0 0 40" fill="none" stroke="rgba(120,130,125,0.25)" stroke-width="1"/>
                        </pattern>
                    </defs>
            ...
            <div class="bg-body-tertiary p-3 border-top">
                <form method="POST" action="<?= lab_url('praktik', 'topologi', ['cidr' => $tp_cidr]) ?>" class="d-flex align-items-center gap-3 m-0 flex-wrap">
                    <?= csrf_field() ?>
                    <input type="hidden" name="topo_op" value="save">
                    <input type="hidden" name="cidr" value="<?= htmlspecialchars($tp_cidr) ?>">
...
function addNode(type, prefix) {
    ...
    nodes.push({ x: 50 + (Math.random() * 200), y: 150 + (Math.random() * 100), label: prefix + ' ' + count, type: type });
    draw();
}
